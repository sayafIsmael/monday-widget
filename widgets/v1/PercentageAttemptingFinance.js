...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    monday.listen("context", res => {
      ...
      monday.api(`query { items_by_multiple_column_values (board_id: 1676895469, column_id: "dropdown", column_values: ["Approval", "Decline", "Partial"]) { name } }`,
        { variables: { boardIds: this.state.context.boardIds } }
      )
        .then(res => {
          console.log("Res data:", res.data.items_by_multiple_column_values.length)
          this.setState({ boardData: res.data, filteredStatusCount: res.data.items_by_multiple_column_values.length });
        });
