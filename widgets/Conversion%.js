...
class App extends React.Component {
  ...
  componentDidMount() {
    ...
    monday.listen("context", res => {
      ...
      monday.api(`{
        items_by_multiple_column_values(board_id: 1676895469, column_id: "status_8", column_values: ["Pipeline"]) {
          name
        }
      }`,
        { variables: { boardIds: this.state.context.boardIds } }
        ...
        .then(res => {
          ...
          this.setState({ pipelinesCount: res.data.items_by_multiple_column_values.length });
        });
  ...
  render() {
    return <div className="App" style={{ background: (this.state.settings.background) }}>
      ...
      >
        ...
        <h2 style={{ fontSize: 75 }}>{(parseFloat((this.state.pipelinesCount / this.state.smartstylesCount ) * 100) || 0).toFixed(1)}%</h2>
      </div>
