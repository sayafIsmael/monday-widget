        ...
        .then(res => {
          ...
          allData.map((item, i) => {
            ...
            allData[i].map(field3 => {
              if (field3.id == "status" && field3.text == "Convert") {
  ...
  render() {
    return <div className="App" style={{ background: (this.state.settings.background) }}>
      ...
      >
        ...
        <h2 style={{ fontSize: 75 }}>{((parseFloat(this.state.totalConverted) / parseFloat(this.state.totalConvertedBlank)) * 100 || 0).toFixed(1)}%</h2>
      </div>
